---
title: TimeCockpitClient - Beispielcode für die Web API
description: Beispielimplementierung der Klasse TimeCockpitClient für die Web API von time cockpit. Mit Beispielen für OData, Query-Endpunkt und POCO-Mapping.
en_page: doc/web-api/timecockpit-client.md
---
# Beispielimplementierung TimeCockpitClient

In diesem Artikel stellen wir einige Hilfsklassen bereit, die Ihnen den Einstieg in die Web API von time cockpit erleichtern sollen. 

- **Klasse TimeCockpitClient:** Diese Klasse kapselt sowohl den OData-Endpunkt für CRUD-Operationen als auch den Query-Endpunkt für anspruchsvollere Datenabfragen.
- **Klasse TcProject:** Diese Klasse soll Ihnen zeigen, wie Sie das Ergebnis einer TCQL-Abfrage über den Query-Endpunkt manuell auf ein POCO abbilden können. Beachten Sie, dass das POCO nicht nur für den Query-Endpunkt, sondern auch als Payload für den OData-Endpunkt verwendet wird.
- **Klasse Settings:** Diese Klasse ist eine stark typisierte Darstellung Ihrer Konfiguration. Sie enthält die Einstellungen aller konfigurierten Konfigurationsanbieter wie appsettings.json, Umgebungsvariablen oder z. B. [KeyVault](https://learn.microsoft.com/en-us/azure/key-vault/general/overview).

## Klasse TimeCockpitClient
