...
Eine Beziehung kann auch nur mit einer BoundCell visualisiert werden. In diesem Fall zeigt time cockpit keinen Hyperlink an, sondern verwendet die Default display property der verknüpften Entität und stellt eine Textzelle dar. `<BoundCell Content="=Current.APP_Customer" />` ergibt also eine Zelle mit dem Firmennamen eines Kunden.
...
Feld | Beschreibung
--- | ---
Content | Im obigen Beispiel verwenden wir nur `Current.Customer` als Inhalt des Hyperlinks. In diesem Fall zeigt time cockpit den Wert der Default display property einer Entität an. Bei der Entität Kunde wäre das `CompanyName`. Benutzer können die Default display property einer Entität natürlich ändern. Würde ein Benutzer die Default display property beispielsweise auf `Code` ändern, würde die Spalte den Code eines Kunden als Hyperlink anzeigen.<br/><br/>![Inhalt des Hyperlinks](/doc/data-model-customization/images/hyperlink-content.png "Inhalt des Hyperlinks")
