...
## Abschnitt Outgoing Invoice

### Rechnungsdatum
Wenn Sie die Aktion `Create Invoice` verwenden, wird das Feld für das Rechnungsdatum automatisch auf das aktuelle Datum gesetzt. Wenn Sie eine Rechnung dagegen manuell erstellen, müssen Sie dieses Feld selbst nach Ihren Vorstellungen setzen.
...
>[!NOTE]
> Die Adresse wirkt sich hinsichtlich der Umsatzsteuer auch auf Ihr Rechnungsdokument aus. Lesen Sie dazu [Rechnungslegung](~/doc/projektzeiterfassung/rechnungslegung.md) – Einen Rechnungsbericht generieren.

## Abschnitt General

### Beginn und Ende des Leistungszeitraums

Beginn- und Enddatum werden auf das Rechnungsdokument gedruckt. Wird die Rechnung mit `Create Invoice` aus ausgewählten Zeitbuchungen erstellt, wird der Leistungszeitraum automatisch anhand der ersten und der letzten Zeitbuchung ermittelt.

### Nettoumsatz

In einer Rechnung entspricht `Net Revenue` den gesamten Erlösen aus allen Rechnungspositionen, die der Rechnung zugeordnet sind. Dieser Wert gibt den erzielten Gesamtumsatz ohne Umsatzsteuer an. 
...
> [!NOTE]
> Frühere Versionen von time cockpit enthielten die Felder `Calculated Revenue` und `Deviating Revenue`. Beide Felder werden für künftige Rechnungen nicht mehr benötigt. Wir empfehlen, die Anzeige dieser Felder in den [globalen Einstellungen](https://web.timecockpit.com/app/lists/entity/APP_GlobalSettings) zu deaktivieren. Bei bestehenden Rechnungen wird der im Feld 'Deviating Net Revenue' eingetragene Wert weiterhin im Feld 'Net Revenue' berücksichtigt. Abweichenden Umsatz und Rechnungspositionen können Sie nicht gleichzeitig in einer Rechnung verwenden.
...
2. Geben Sie im Feld `Discount [%]` den Prozentsatz des Skontos ein, das Sie dem Kunden gewähren.
...
>[!NOTE]
> Wenn Sie in der Rechnung das Datum `Payment Received On` setzen und dieses innerhalb der Skontofrist liegt, befüllt time cockpit das Feld `Payment Amount` automatisch mit dem Wert von `Gross Revenue Discounted`.

## Abschnitt Invoice Report
...
Zusätzlich legen Sie mit dem Kennzeichen `Incl. Timesheets on Invoice Report` fest, ob der Rechnungsbericht für jede Rechnung die Details der Zeitbuchungen enthalten soll. Mit diesem Kennzeichen steuern Sie den Detaillierungsgrad Ihres Rechnungsberichts.
